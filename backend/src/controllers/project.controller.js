...
export const getProjects = async (req, res) => {
  const { page = 1, limit = 12, status, category, featured, search, sort = '-createdAt' } = req.query
  ...
  res.json({ projects, pagination: { total, page: Number(page), pages: Math.ceil(total / limit), limit: Number(limit) } })
}
...
export const adminGetProjects = async (req, res) => {
  ...
  const projects = await Project.find(filter)
    ...
    .limit(Number(limit))
    .populate('createdBy', 'name')
  res.json({ projects, pagination: { total, page: Number(page), pages: Math.ceil(total / limit) } })
}
